...
自然语言方向另由入口的语义一致性层复核：向前/后是 `dx` 正/负，向左/右
是 `dy` 正/负，向上/下是 `dz` 正/负；明确距离按 mm/cm/m 换算。它只防止
LLM 误解原话，不替代工作空间、IK、碰撞或运行期保护。正式主入口在启动
时确认真机风险，不对每条命令重复询问 `y/N`；该确认分支仅供测试注入。
...
目标需要通过网格有效节点/完整单元。运动优先从当次实际姿态直接到目标，
完整最终执行 waypoint 经过 MuJoCo、关节和接触预检；直接路线未通过且
尚未运动时才回退到 `middle_internal` 中心通道。执行前还会核对真实起点。工作空间与
