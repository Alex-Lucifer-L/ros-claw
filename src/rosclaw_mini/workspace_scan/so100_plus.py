"""SO-100 Plus 离线不规则工作空间扫描实现。

每个笛卡尔网格点都从参考关节姿态独立求解，并检查端点及从参考姿态
到目标关节角的插值路径。本模块不访问任何真实设备。

命令行兼容入口保留在 ``scripts/simulate_so100_plus_rest_workspace.py``；
未来程序或 LLM 工具应导入本包，而不是反向依赖 scripts 目录。
"""
...
REFERENCE_JOINT_RADIANS = {
    "joycon_initial": SO100_PLUS_JOYCON_INITIAL_RADIANS,
    "middle_internal": SO100_PLUS_MIDDLE_INTERNAL_RADIANS,
}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--reference-pose",
        choices=tuple(REFERENCE_JOINT_RADIANS),
        default="joycon_initial",
        help="固定 TCP 朝向和所有路径的参考姿态；默认 joycon_initial",
    )
    parser.add_argument(
        "--gripper-driver-degrees",
        nargs="+",
        type=float,
        default=[0.0],
        help=(
            "需要同时无碰撞的一个或多个已校准夹爪驱动角；"
            "多个值会取共同安全交集"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="可选精扫边界，必须包含参考点且位于候选外包围盒内",
    )
    parser.add_argument(
        "--allow-bounds-outside-candidate-aabb",
        action="store_true",
        help=(
            "仅用于纯离线探边：允许显式精扫范围超出旧随机样本 "
            "AABB；仍保留 IK、关节、碰撞和路径检查"
        ),
    )
    return parser
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    gripper_values = tuple(args.gripper_driver_degrees)
    if not gripper_values or any(
        not math.isfinite(value) for value in gripper_values
    ):
        raise SystemExit("gripper-driver-degrees 必须包含有限数值。")
...
def centered_axis_values(
    ...
    if len(center_indices) != 1:
        raise RuntimeError("生成的坐标轴没有唯一参考网格点。")
...
def pose_has_collision(
    ...
    *,
    gripper_qpos: float = GRIPPER_MODEL_RADIANS,
) -> bool:
    ...
    data.qpos[ARM_JOINT_COUNT] = gripper_qpos
...
def select_grid_bounds(
    ...
    *,
    allow_outside_candidate_aabb: bool = False,
    ...
    if not allow_outside_candidate_aabb and (
        np.any(lower < candidate_lower - 1e-12)
        or np.any(upper > candidate_upper + 1e-12)
    ):
        raise ValueError("bounds-m 不能超出所有姿态候选外包围盒。")
    if np.any(rest < lower - 1e-12) or np.any(rest > upper + 1e-12):
        raise ValueError("bounds-m 必须包含参考 TCP。")
    if lower[2] < 0:
...
def _save_plot(
    ...
    *,
    reference_label: str = "JoyCon controller initial",
) -> None:
    import matplotlib
    ...
    view3d.scatter(
        ...
        marker="*",
        s=100,
        label=f"{reference_label} TCP",
    )
    view3d.set(
        xlabel="X (m)",
        ylabel="Y (m)",
        zlabel="Z (m)",
        title=f"{reference_label}-orientation path-valid grid",
    )
    view3d.legend(loc="upper right")
    figure.suptitle(
        f"SO-100 Plus {reference_label}-orientation candidate workspace\n"
...
def main() -> int:
    ...
    reference_name = args.reference_pose
    reference_label = (
        "middle_internal"
        if reference_name == "middle_internal"
        else "JoyCon controller initial"
    )
    rest_qpos = np.asarray(
        REFERENCE_JOINT_RADIANS[reference_name],
        dtype=float,
    )
    ...
    if gripper_joint_id < 0:
        raise RuntimeError("MuJoCo 模型缺少 gripper_joint。")
    gripper_qpos_range = tuple(
        float(value) for value in model.jnt_range[gripper_joint_id]
    )
    gripper_driver_degrees = tuple(
        float(value) for value in args.gripper_driver_degrees
    )
    gripper_qpos_values = tuple(
        so100_plus_gripper_driver_degrees_to_mujoco_qpos(value)
        for value in gripper_driver_degrees
    )
    for driver_degrees, gripper_qpos in zip(
        gripper_driver_degrees,
        gripper_qpos_values,
        strict=True,
    ):
        if not gripper_qpos_range[0] <= gripper_qpos <= gripper_qpos_range[1]:
            raise RuntimeError(
                f"夹爪驱动角 {driver_degrees:.6f}° 映射为 "
                f"{gripper_qpos:.6f} rad，超出模型范围 "
                f"[{gripper_qpos_range[0]:.6f}, "
                f"{gripper_qpos_range[1]:.6f}]。"
            )
        if pose_has_collision(
            model,
            data,
            ...
            raise RuntimeError(
                f"{reference_label} 在夹爪 {driver_degrees:.3f}° 时"
                "发生碰撞。"
            )
    ...
    grid_lower, grid_upper, bounds_source = select_grid_bounds(
        ...
        candidate_upper,
        args.bounds_m,
        allow_outside_candidate_aabb=(
            args.allow_bounds_outside_candidate_aabb
        ),
    )
        ...
        for y_index, y in enumerate(y_values):
            for z_index, z in enumerate(z_values):
                ...
                if any(
                    ...
                    for gripper_qpos in gripper_qpos_values
                ):
                    status[
                        x_index,
                        y_index,
    ...
    np.savez_compressed(
        ...
        valid_tcp_points_m=valid_positions.astype(np.float32),
        gripper_driver_degrees=np.asarray(
            gripper_driver_degrees,
            dtype=float,
        ),
        gripper_qpos_radians=np.asarray(
            gripper_qpos_values,
            dtype=float,
        ),
    )
    ...
    report = {
        "classification": (
            f"{reference_name}_orientation_irregular_grid_candidate_"
            "not_real_hardware_certification"
        ),
        ...
        "inputs": {
            ...
            "reference_pose": reference_name,
            "tcp_orientation": f"{reference_label} rotation matrix",
            "gripper_driver_degrees": list(gripper_driver_degrees),
            "gripper_qpos_radians": list(gripper_qpos_values),
            "gripper_combination": "intersection_all_listed_postures",
            "allow_bounds_outside_candidate_aabb": (
                args.allow_bounds_outside_candidate_aabb
            ),
        },
        "grid": {
        ...
        "path_check": {
            "start": f"{reference_label} joint position",
        ...
        "limitations": [
            "The irregular reachable set is sampled only at Cartesian grid points.",
            f"Every valid point was checked from {reference_name}.",
            f"The TCP orientation is fixed to {reference_name}.",
            "Every valid point and path passed for all listed gripper postures.",
            "Real backlash, sag, cable routing, payload, and thermal effects are absent.",
            "Do not copy the irregular AABB into real WorkspaceLimits.",
        ],
    }
    ...
    print(f"固定 {reference_label} 姿态网格结果：", flush=True)
    for label, count in counts.items():
        print(f"  {label}: {count}", flush=True)
    print("包含参考点的最大全通过网格长方体：", flush=True)
    for axis in ("x", "y", "z"):
