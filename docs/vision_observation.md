...
`bounding_box` 是画面区域，不能单独作为三维坐标或抓取点。
普通 Webcam 观察仍使用 VLM 的 0–1 归一化语义框。可选 RealSense
定位链使用更严格的专用协议：VLM 返回原图整数像素框，本地代码验证后
才归一化，再作为同帧对齐 Depth 的 ROI。它仍会检查有效深度率、
离群点和离散程度。解析器会拒绝非法枚举、置信度、边界框以及机械臂/基座
...
V2.0默认入口仍只实现“看”。独立的可选RealSense链已经实现RGBD同步、
Depth对齐、稳健目标深度、相机光学坐标和固定相机外参的离线点对求解，
详见[RealSense RGBD目标定位与固定相机外参](realsense_localization.md)。
真实外参尚未采集和激活，因此像素到机械臂基座的现场验收、`SceneState`、
`TaskPlan`、自动抓取、视觉伺服、连续视频推理和VLM自动控制机械臂仍未开放。

要进入抓取阶段，仍必须完成真实eye-to-hand点对、留出点误差、基座坐标
尺量验收和Mock抓取计划。不能把物体描述、画面语义区域或未经深度质量
检查的bounding box直接连接到现有运动Skill。
