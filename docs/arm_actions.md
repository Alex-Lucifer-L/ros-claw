...
每个工作目标都按 `当前实测关节 → middle_internal → 目标` 生成中心通道。
精确网格点使用产物中保存的关节解，连续点从 `middle_internal` 重新求 IK；
最终 30 Hz waypoints 在预检前一次性固化，MuJoCo 验证和 Adapter 执行使用
同一组轨迹。
...
`stop` 都不会自动恢复或自动重试。`exit` 不会从 WORK 或
