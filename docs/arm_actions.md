...
- `unfold_arm` 使用当次实际读取并通过容差检查的 follower_rest 关节角，
  先规划完整的 `REST → storage_escape → 工作初始姿态`；
- 检查原有收纳接触之外是否出现新接触、`storage_escape` 是否已经脱离
  接触、escape 后是否持续无接触，以及工作初始姿态是否无接触；
- 同时检查收纳通道的 TCP 方向、关节范围和相邻关节步长；返回工作
  初始姿态的普通无接触路径还检查 TCP 不低于支撑平面；
- `fold_arm` 从当前实际 WORK 姿态规划返回工作初始姿态的完整关节路径，
  逐个不超过 1° 的 MuJoCo 采样点检查无碰撞；到位门禁通过后，再对
  `工作初始姿态 → storage_escape → follower_rest` 做反向完整检查；
- 只有整段预检查全部通过才开始该段运动，Adapter 接收的是验证器原样
  返回的同一个 `JointMotionPlan`，不会再调用 `move_to()` 规划另一条路。
