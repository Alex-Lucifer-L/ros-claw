...
class ManipulatorRobot:
    ...
    def set_so100_robot_preset(self):
        for name in self.follower_arms:
            ...
            self.follower_arms[name].write("D_Coefficient", 0) # 32
        ...
        for name in self.follower_arms:
            ...
            if self.controllers is not None:
                ...
                for idx in range(len(Present_Load)):
                    if abs(Present_Load[idx]) >= 1024:
                        Present_Load[idx] = abs(1024 - abs(Present_Load[idx]))
                print("Present_Load", [f"{x:.3f}" for x in Present_Load])
                # print(f'{type(goal_pos)=}, {type(feedback)=}')
                # if self.gripper_state[name] <= 0 :
                if Present_Load[-1] > 300:
